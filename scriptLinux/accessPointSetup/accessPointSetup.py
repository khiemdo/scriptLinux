...
def EditIp4_forward():
    ...
    pc = subprocess.Popen('iptables -t nat -A POSTROUTING -o eth0 -j MASQUERADE')
    BashHelper.CheckOutputOfCallingBash(pc,logger)

    logger.info('iptables -A FORWARD -i eth0 -o wlan0 -m state --state RELATED,ESTABLISHED -j ACCEPT')
    pc = subprocess.Popen('iptables -A FORWARD -i eth0 -o wlan0 -m state --state RELATED,ESTABLISHED -j ACCEPT')
    BashHelper.CheckOutputOfCallingBash(pc,logger)

    logger.info('iptables -A FORWARD -i wlan0 -o eth0 -j ACCEPT')
    pc = subprocess.Popen('iptables -A FORWARD -i wlan0 -o eth0 -j ACCEPT')
    BashHelper.CheckOutputOfCallingBash(pc,logger)

    logger.info('sh -c "iptables-save > /etc/iptables.ipv4.nat"')
    pc = subprocess.Popen('sh -c "iptables-save > /etc/iptables.ipv4.nat"')
    BashHelper.CheckOutputOfCallingBash(pc,logger)
...
#    filePath = path.join(SCRIPT_DIR, CONFIG_FILE_NAME) 
    ...
    pc = subprocess.Popen('sh -c "echo 1 > /proc/sys/net/ipv4/ip_forward')
    BashHelper.CheckOutputOfCallingBash(pc,logger)
    logger.info('/usr/sbin/hostapd /etc/hostapd/hostapd.conf')
    pc = subprocess.Popen('/usr/sbin/hostapd /etc/hostapd/hostapd.conf')
    BashHelper.CheckOutputOfCallingBash(pc,logger)
    logger.info('service hostapd start')
    pc = subprocess.Popen('service hostapd enable')
    BashHelper.CheckOutputOfCallingBash(pc,logger)
    logger.info('service isc-dhcp-server start')
    pc = subprocess.Popen('service isc-dhcp-server enable')
    BashHelper.CheckOutputOfCallingBash(pc,logger)
    logger.info('service hostapd start')
    pc = subprocess.Popen('service hostapd enable')
    BashHelper.CheckOutputOfCallingBash(pc,logger)
